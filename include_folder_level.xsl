...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template match="/">
        <xsl:apply-templates select="descendant::e:folder[@title=$foldername]" />
    </xsl:template>

    <xsl:template match="e:folder">
        <xsl:apply-templates select="e:component[@title=$startingheading]" />
    </xsl:template>
